...
def my_overtime(
    ...
    rows = db.query(models.OvertimeRequest).filter(
        models.OvertimeRequest.user_id == user.id
    ).all()
...
def overtime_request_submit(
    ...
    if hours < 0.5 or hours > 12:
        return RedirectResponse(
            url="/overtime/request?error=Hours+must+be+between+0.5+and+12",
            status_code=303,
        )

    if ot_date.weekday() >= 5 and not reason:
        return RedirectResponse(
            url="/overtime/request?error=OT+on+weekend+requires+reason",
            status_code=303,
        )
...
def approve_overtime(
    ...
    if employee:
        employee.replacement_leave_balance = (
            employee.replacement_leave_balance or 0
        ) + days
